...
const SearchList = (props) => {
    ...
    const sendAddFriendRequest = async(selectedUser) => {
        const url = `https://melon-web-project-server.vercel.app/friends/send/request/${userId}/${selectedUser._id}`;
        await fetch(url, {
            method: 'POST',
            headers: {
    ...
    const fetchData = async() => {
        const url = `https://melon-web-project-server.vercel.app/users/${props.input}`;
        await fetch(url, {
            method: 'GET',
            headers: {
                Authorization: `Bearer ${token}`,
